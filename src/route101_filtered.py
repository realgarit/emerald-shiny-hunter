#!/usr/bin/env python3
"""
Pokémon Emerald Shiny Hunter - Route 101 (Filtered - No Wurmple)
Uses mGBA Python bindings to hunt for shiny wild Pokémon on Route 101
FILTERED VERSION: Only checks Poochyena and Zigzagoon for shinies (skips Wurmple)

Loads from .sav file and presses buttons to trigger wild encounters.
Identifies Pokemon species from memory and only checks for shiny if it's Poochyena or Zigzagoon.
Automatically resets if Wurmple is encountered.

Features:
- Identifies Pokemon species from memory (Poochyena, Zigzagoon, or Wurmple)
- Only checks for shiny if Poochyena or Zigzagoon (skips Wurmple)
