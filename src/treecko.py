...
Treecko is to the left of Torchic, selected with A dialogue -> Left -> A select.
"""
...
# Selection sequence constants for Treecko (left of center)
# 20 A dialogue -> wait 0.5s -> 1x Left -> wait 0.2s -> 6 A select
A_PRESSES_DIALOGUE = 20
WAIT_FOR_BAG_FRAMES = 30
LEFT_PRESS_COUNT = 1
LEFT_PRESS_DELAY_FRAMES = 15
WAIT_AFTER_LEFT_FRAMES = 12
A_PRESSES_SELECT = 6
...
class ShinyHunter(EmulatorBase):
    """Shiny hunter for Treecko starter."""

    def __init__(self, suppress_debug=True, show_window=False):
        # Initialize logging
        self.log_dir = PROJECT_ROOT / "logs"
        self.log_manager = LogManager(self.log_dir, "treecko_hunt")
        ...
        super().__init__(
            ...
            window_name="Shiny Hunter - Treecko"
        )
        ...
        print(f"[*] Target: Treecko (left position)")
    ...
    def selection_sequence(self, verbose=False):
        """
        Execute the selection sequence for Treecko.
        ...
        if verbose:
            print(f"[*] {A_PRESSES_DIALOGUE} A dialogue -> wait {wait_bag_seconds:.1f}s -> "
                  f"{LEFT_PRESS_COUNT}x Left -> wait {wait_left_seconds:.1f}s -> {A_PRESSES_SELECT} A select")
        ...
        for _ in range(LEFT_PRESS_COUNT):
            ...
            self.run_frames(LEFT_PRESS_DELAY_FRAMES)
        if verbose:
            print(" Done")

        # Step 4: Wait after Left
        if verbose:
