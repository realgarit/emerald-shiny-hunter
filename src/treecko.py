#!/usr/bin/env python3
"""
Pokémon Emerald Shiny Hunter - Treecko
Uses mGBA Python bindings to hunt for shiny Treecko starter

Loads from .sav file and presses buttons to select Treecko.
...
# Button sequence for Treecko (based on Mudkip sequence, using Left instead of Right)
# Sequence: 20 A dialogue -> wait 0.5s -> 1x Left -> wait 0.2s -> 6 A select
# Treecko is to the left of Torchic, so we press Left to move to Treecko
# Note: Same sequence as Mudkip but using Left button instead of Right
...
LEFT_PRESS_COUNT = 1  # Press Left 1 time to move to Treecko
LEFT_PRESS_DELAY_FRAMES = 15  # Wait 0.25s after Left press (standard delay)
WAIT_AFTER_LEFT_FRAMES = 12  # Wait 0.2s after Left press before A presses
A_PRESSES_SELECT = 6  # A presses to select Treecko (6 is optimal balance)
...
class ShinyHunter:
    ...
    def selection_sequence(self, verbose=False):
        """Execute the full selection button sequence for Treecko
        
        Sequence (based on Mudkip sequence, using Left instead of Right):
        20 A dialogue -> wait 0.5s -> 1x Left -> wait 0.2s -> 6 A select
        Treecko is to the left of Torchic, so we press Left to move to Treecko
        """
        delay_seconds = A_PRESS_DELAY_FRAMES / 60.0
        wait_bag_seconds = WAIT_FOR_BAG_FRAMES / 60.0
        wait_left_seconds = WAIT_AFTER_LEFT_FRAMES / 60.0
        
        if verbose:
            print(f"[*] {A_PRESSES_DIALOGUE} A dialogue -> wait {wait_bag_seconds:.1f}s -> {LEFT_PRESS_COUNT}x Left -> wait {wait_left_seconds:.1f}s -> {A_PRESSES_SELECT} A select")
        ...
        # Step 3: Press Left to move to Treecko
        if verbose:
            print(f"    Pressing Left {LEFT_PRESS_COUNT} time(s)...", end='', flush=True)
        for _ in range(LEFT_PRESS_COUNT):
            self.press_left(hold_frames=5, release_frames=5)
            self.run_frames(LEFT_PRESS_DELAY_FRAMES)  # Use specific delay for Left presses
        if verbose:
            print(" Done")
        
        # Step 4: Wait after Left to ensure cursor moved
        if verbose:
            print(f"    Waiting {wait_left_seconds:.2f}s after Left...", end='', flush=True)
        self.run_frames(WAIT_AFTER_LEFT_FRAMES)
        if verbose:
            print(" Done")
        
        # Step 5: Press A buttons to select Treecko
        if verbose:
